...
- `wave_overhang_travel_speed` and `wave_overhang_fan_speed`: saved per-profile, not yet applied per-path. Print-speed override works.
- `wave_overhang_min_angle`: saved per-profile, not yet gating generator entry (angle estimator pending).
- `support_remaining_areas_after_wave_overhangs`: saved, support pipeline integration pending.
